...
namespace Naninovel.Runtime.UI
{
    public class FontChanger : MonoBehaviour
    {
        ...
        private void OnEnable ()
        {
            ...
            if (uiManager.FontName != null)
            {
                var fontOption = uiManager.Configuration.GetFontOption(uiManager.FontName);
                ChangeFont(fontOption?.Font, fontOption?.TMPFont, configuration);
            }
            if (uiManager.FontSize >= 0)
                ChangeFontSize(uiManager.FontSize, configuration);
        }
        ...
        private void HandleFontNameChanged (string fontName)
        {
            var fontOption = uiManager.Configuration.GetFontOption(fontName);
            ChangeFont(fontOption?.Font, fontOption?.TMPFont, configuration);
        }

        private void HandleFontSizeChanged (int sizeIndex)
        {
